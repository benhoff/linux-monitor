...
On `pacman` systems it groups the dashboard into three core tabs plus package tabs:
...
On non-`pacman` systems such as Debian, the core health tabs still work and Arch-specific package tabs are hidden.
...
- The package panel tracks only kernel, firmware, and NVIDIA versions, and is shown only on `pacman` systems.
